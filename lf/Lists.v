...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  |[] => []
  |x :: ls => match (even x) with
              |true => (oddmembers ls)
              |false => x :: (oddmembers ls)
              end
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  |h1 :: t1 => match count h1 s2 with
               |0 => false
               |_ => included t1 (remove_one h1 s2)
               end
  |[] => true
  end.
...
Theorem succ_n_eq_plus_n_1: forall n,
  S n = n + 1.
Proof.
  intros; induction n.
...
Proof.
  intros.
  induction n.
  - simpl. rewrite succ_n_eq_plus_n_1. reflexivity.
  - induction s.
    + simpl.   
Admitted.
