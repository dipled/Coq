...
(*O COC CONSEGUE INFERIR OS TIPOS DA SEGUINTE FORMA:::::*)
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => andb (negb (x <=? 7)) (even x)) l.
...
Definition compose {X Y Z : Type} (g : Y -> Z) (f : X -> Y) (x : X) : Z :=
  g(f x).
...
Lemma map_dist_app : forall (X Y : Type) (f: X -> Y) (l1 l2 : list X),
...
Proof.
  intros. induction l as [| h t IHl].
  - reflexivity.
  - simpl. rewrite map_dist_app. simpl. rewrite IHl. reflexivity.
Qed. 
(** [] *)
...
Proof.
  intros. unfold prod_uncurry. unfold prod_curry. simpl. reflexivity. 
Qed. 
...
Proof.
  intros.
  destruct p. unfold prod_curry. unfold prod_uncurry. simpl. reflexivity.
Qed.
(** [] *)
